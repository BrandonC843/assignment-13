...
// INPUT: ARRAY OF PBJECTS & CRITERIA OBJECT FOR VALLID PROPS/VALUES
var where = function(arrayOfObjects, criteriaObject){
var fiteredListOfObjects = []

   for(var i = 0; i < arrayOfObjects.length ; i = i + 1){
      var singleObject = arrayOfObjects[i]
      var objectMatchesAllCriteria = true


   for(var propString in criteriaObject){
      if(criteriaObject[propString] !== singleObject[propString] ){
         objectMatchesAllCriteria = false
      }
   }
      if(objectMatchesAllCriteria){
         fiteredListOfObjects.push(singleObject)
      }

}

return fiteredListOfObjects
}
